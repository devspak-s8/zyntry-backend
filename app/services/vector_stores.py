...
class PgVectorStore(BaseVectorStore):
        ...
        for vec in vectors:
            await self._session.execute(
                text(
                    ...
                    VALUES (:id, :project_id, :document_id, CAST(:vector AS jsonb), :model, :dimensions, CAST(:metadata AS jsonb), :external_id)
    ...
    async def search(self, query_vector: list[float], limit: int = 10, filters: dict | None = None, query_text: str | None = None) -> list[dict]:
        ...
        result = await self._session.execute(
            text(
                f"SELECT id, project_id, document_id, vector, metadata "
                f"FROM {self._table_name} {filter_clause}"
            ),
            params,
        )
        ...
        for row in result.fetchall():
            candidate = [float(value) for value in (row.vector or [])]
            candidate_magnitude = math.sqrt(sum(value * value for value in candidate))
            score = 0.0
            if query_magnitude and candidate_magnitude:
                score = sum(a * b for a, b in zip(query_vector, candidate)) / (
                    query_magnitude * candidate_magnitude
                )
