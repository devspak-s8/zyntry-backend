    ...
    async def execute(self, action: str, arguments: dict[str, Any], context: dict[str, Any]) -> ActionResponse:
        ...
        try:
            ...
            if action == "list_messages":
                ...
                thread_id = original_data.get("threadId", arguments["message_id"])
                message = f"From: me\nTo: {original_data.get('payload', {}).get('headers', [{}])[0].get('value', '')}\nSubject: Re: {original_data.get('payload', {}).get('headers', [{}])[1].get('value', '')}\n\n{arguments['body']}"
                import base64
                encoded = base64.urlsafe_b64encode(message.encode()).decode()
                resp = await client.post(f"{self._base_url}/gmail/v1/users/me/messages/send", headers=headers, json={"raw": encoded, "threadId": thread_id})
                return ActionResponse(success=resp.status_code in (200, 201), result=resp.json(), error=str(resp.text) if resp.status_code not in (200, 201) else None)
                ...
                resp = await client.post(f"{self._base_url}/gmail/v1/users/me/messages/{arguments['message_id']}/archive", headers=headers)
                return ActionResponse(success=True, result=resp.json())
                ...
                resp = await client.post(f"{self._base_url}/gmail/v1/users/me/messages/{arguments['message_id']}/modify", headers=headers, json={"removeLabels": ["UNREAD"]})
                return ActionResponse(success=True, result=resp.json())
